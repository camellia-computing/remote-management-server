...
`0016_recording_encryption`是明确的no-legacy前滚迁移：只允许空的recording inventory，发现任何旧`RecordingUpload`行会在DDL前拒绝升级，绝不会把已有明文文件静默标为已加密。本项目尚未发布，因此不提供不受审的在线明文转换器；升级已有开发/测试环境前必须停写并按数据处置要求隔离或安全销毁旧record volume，然后从一致的空数据库和空record volume重建。不得只删upload/usage行、保留旧volume继续启动，亦不得绕过migration ledger。需要保留的旧录像只能在独立离线隔离环境走另行审查的转换流程，不能重新挂入生产record root。
